...
res = session.post('http://127.0.0.1:5000/register', data={'username': 'testuser10', 'email': 'testuser10@test.com', 'password': 'Password123', 'csrf_token': csrf})
print('Register status:', res.status_code)

# Login
csrf = get_csrf('http://127.0.0.1:5000/login')
res = session.post('http://127.0.0.1:5000/login', data={'email': 'testuser10@test.com', 'password': 'Password123', 'csrf_token': csrf})
...
if res.status_code == 200:
    ...
    if m:
        ...
        print('\nMISSING SKILLS NOT FOUND IN HTML')
