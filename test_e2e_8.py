...
csrf = m.group(1)
client.post('/register', data={'username': 'testuser24', 'email': 'testuser24@test.com', 'password': 'Password123', 'csrf_token': csrf})

res = client.get('/login')
m = re.search(r'name="csrf_token" value="(.*?)"', res.text)
csrf = m.group(1)
client.post('/login', data={'email': 'testuser24@test.com', 'password': 'Password123', 'csrf_token': csrf})
...
if match:
    ...
    print('HTML EXTRACT:', res.text[:2000])
